...
dvar float+ h_p1[1..cp1][i in 1..4];
dvar float+ h_p2[1..cp2][i in 1..4];
dvar float+ h_p3[1..cp3][i in 1..4];
...
//最小化住院医师和主治医师值班的次数
minimize 
		sum(k in 1..cp1)sum(n in 1..N)X1[k][n][S[4]] +
		sum(k in 1..cp2)sum(n in 1..N)X2[k][n][S[4]];

//最小化住院医师值班的次数
//minimize 
//		sum(k in 1..cp1) sum(n in 1..N)X1[k][n][S[4]];
		//sum(k in 1..cp3)sum(n in 1..N)X1[k][n][S[4]];
//****************************约束****************************\\
subject to{
// H1
forall(k in 1..cp1)
...
//      sum(i in n..n+N_max)X1[k][i][S[2]] - h_p1[k][3] <= N_max;
//forall(k in 1..Cp[2])
//    forall(n in 1..N-N_max)
//      sum(i in n..n+N_max)X2[k][i][S[2]] - h_p2[k][3] <= N_max;
//forall(k in 1..Cp[3])
//    forall(n in 1..N-N_max)
//      sum(i in n..n+N_max)X3[k][i][S[2]] - h_p3[k][3] <= N_max;
//
//// S6
//forall(k in 1..Cp[1])
//    forall(n in 1..N-W_max)
//      sum(i in n..n+W_max)O1[k][i] - h_p1[k][4] <= W_max;
//forall(k in 1..Cp[2])
//    forall(n in 1..N-W_max)
//      sum(i in n..n+W_max)O2[k][i] - h_p2[k][4] <= W_max;
//forall(k in 1..Cp[3])
//    forall(n in 1..N-W_max)
//      sum(i in n..n+W_max)O3[k][i] - h_p3[k][4] <= W_max;
